...
#include <stdint.h>  // Standard Integer Types in ISOC99
#include <stdio.h>
...
int main()
{
   ...
   character in format strings to indicate "unsigned". */
	...
	/* Print limits of 8-bit integers, signed and unsigned.  */

	printf("\nsizeof(int8_t) = %10ld bytes,  %ld bits\n", sizeof(int8_t), sizeof(int8_t) * 8);
	printf("INT8_MAX      = %+11d\n", INT8_MAX );
	printf("INT8_MIN      = %+11d\n", INT8_MIN );
	printf("\nsizeof(uint8_t) = %9ld bytes,  %ld bits\n", sizeof(uint8_t), sizeof(uint8_t) * 8);
	printf("UINT8_MAX     = %+11d\n", UINT8_MAX);
	printf("UINT8_MIN     = %+11d\n", 0);
	...
	printf("SHRT_MAX      = %+11hi\n", SHRT_MAX); // %...hi for Signed integer (short)
	printf("SHRT_MIN      = %+11hi\n", SHRT_MIN);
...
character in format strings to indicate "unsigned". */
	...
	/* display the same binary memory location as a character or number */
	int p;
	for (p = 0; p <= 255; p++)
	{
		printf("%c is ASCII %d\n", p, p);
		// %c interprets binary in p memory location as an alphanumeric ASCII character 
		// %d interprets binary in p memory location as a base 10 numeric value
	}

	printf("print and compare the same binary value as a character or number\n");
	char charVar = 'U';
	int8_t int8Var = 85;
	printf("  char %c == int8_t %d is %s\n", charVar, int8Var, (charVar == int8Var ? "TRUE" : "FALSE"));
	printf("int8_t %c ==   char %d is %s\n", int8Var, charVar, (int8Var == charVar ? "TRUE" : "FALSE"));

	printf("\ncompare unsigned versus signed integers\n");
	int i = -1;
	unsigned int unI = INT_MAX;
	printf("\t%u == %d is %s\t%s\n", i, unI, (i == unI ? "true" : "false"), "CORRECT");
	printf("\t%u >  %d is %s\t%s\n", i, unI, (i > unI ? "true" : "false"), "CORRECT");
	printf("\t%u <  %d is %s\t%s\n", i, unI, (i < unI ? "true" : "false"), "CORRECT");
	unI = UINT_MAX;
	printf("\t%u == %d is %s\t%s\n", i, unI, (i == unI ? "true" : "false"), "WRONG");
	printf("\t%u >  %d is %s\t%s\n", i, unI, (i > unI ? "true" : "false"), "WRONG");
	printf("\t%u <  %d is %s\t%s\n", i, unI, (i < unI ? "true" : "false"), "CORRECT");
	printf("\n");
	...
	printf("maximum value  = 2^%d = %.0f\n", FLT_MANT_DIG, pow(2, FLT_MANT_DIG));
	printf("significant digits accuracy = min. %u positions of whole + fractional parts. i.e. the number of base-10 digits that can be represented accurately.\ne.g. 0.123456 or 123456.0 or 123.456\n", FLT_DIG);
